      ...
      },
      {
        ...
        render: (text, record) =>
          this.props.users.length >= 1 ? (
            <Popconfirm title="Sure to delete?" onConfirm={() => this.handleDeleteEmit(record._id)}>
              <a>Delete</a>
            </Popconfirm>
          ) : null,
      },
  ...
  socket.on('changeUser', (data) => {
      ...
      this.props.users.find(item => {
        if(item._id == data._id) {
           item[`${data.inputId}`] = data.newValue[`${data.inputId}`];
...
handleAdd = (obj) => {
  const newUser = {name: obj.name, email: obj.email, number: obj.number};
  console.log(newUser);
  this.props.addUser(newUser);
}
handleAddEmit = (obj) => {
  socket.emit('addUser', {obj: obj}, (data) => {
    this.handleAdd(obj);
    console.log(data);
});
}
...
handleDeleteEmit = (id) => {
  socket.emit('deleteUser', {id: id}, (data) => {
...
handleFetch = () => {
  ...
  .then(response => {
    ...
    this.props.setUsers({users: response.users, pageCount: response.pageCount});
    console.log(this.props)
  })
...
handleSearch = debounce((value) => {
  ...
  render() {
    ...
    />
        <Modal1 handleAdd={this.handleAddEmit} style={{marginLeft: '30px'}} size={'lg'}/>
        </div>
      ...
      <span>Page number: </span>{
        ...
        <PageSelect number={this.props.pageCount} handleSelectPage={this.handleSelectPage}/>
      </span>
        </span>
       </div>
    );
